...
class VigenerePeriodClassifier(object):
    def __init__(self, model_file="vigenere_period_model.joblib", scaler_file="vigenere_period_scaler.joblib",
                 retrain=False, training_file="vigenere_period_reference_data.csv",
    ...
    def identify(self, text):
        return list(sorted(zip(map(int, self.model.classes_), self.model.predict_proba(
            self.scaler.transform(np.array(self.stats.periodic_ioc(text)[1:]).reshape(1, -1)))[0]),
                           key=lambda x: x[1], reverse=True))

if __name__ == '__main__':
    VigenerePeriodClassifier(retrain=True)